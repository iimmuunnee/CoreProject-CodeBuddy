...
const handleMessageSubmit = (event) => {
  ...
  // 메시지 전송
  // console.log(typeof(chatSocket)) // object
  chatSocket.emit("new_message", { message : message, roomName, nickname }, () => {addMessage(nickname, message)});
...
chatSocket.on("new_message", ({ nickname, message }) => {
...
chatSocket.on("welcome", ({ nickname, newCount }) => {
...
chatSocket.on("user_count", ({room_name, user_count}) => {
  console.log(`user_count 이벤트의 사용자 수: ${user_count}`);
  $c_content_num.textContent = `${user_count}`
})

chatSocket.on("bye", ({ nickname, newCount }) => {
